...
const HOSTNAME = (import.meta.env?.DEV ? "http://localhost:4805" : "");
...
function updateTrackedCuePositionFromViewport() {
    if (!cueEditorDialog.open || !cuePositionTracking) return;
...
function updateCueEditorPositionMarkers() {
    ...
    if (!cueEditorDialog.open) {
        startLine.hidden = true;
        endLine.hidden = true;
        return;
    }
...
function startAnnotationMode() {
    ...
    if (annotationToolbar) {
        annotationToolbar.color = departmentDefaultColor();
...
function startAnnotationGesture(e, options = {}) {
    ...
    if (gestureTool === 'erase') {
        recordEraserSample(e);
        eraseAnnotationsNear(e.clientX, e.clientY);
        scheduleAnnotationSync(500);
        return true;
    }
    ...
    const annColor = annotationToolbar ? annotationToolbar.color : '#ffd000';
    ...
    if (annotationTool === 'text') {
        ...
        if (text && text.trim()) {
            const ann = {
                ...
                width: annWidth, fontPx: currentScriptFontPx(),
...
function handleDepartmentSettingsEvent(event) {
    ...
    if (settingsDialog.open) setSettingsSaveStatus("Updated from central settings.");
}
...
async function loadShowScript(id) {
    ...
    if (activeDepartment) {
        try {
            ...
            setSyncStatus(activeDepartment + ": cue load error", "error");
        }
        try {
            await loadDepartmentAnnotations();
...
function updateStatus() {
    const text = (playing ? "Playing" : "Paused") + " | Speed: " + speed.toFixed(1);
...
function scheduleToolbarHide() {
    ...
    if ((settingsDialog && settingsDialog.open) || (syncMode === "follow" && !followingLive)) {
        return;
    }
...
if (toolbarSync) {
    ...
    toolbarSync.addEventListener("take-control", async (e) => {
        masterKey = e.detail.password || (toolbarSync ? toolbarSync.getPasswordValue() : "");
    ...
    toolbarSync.addEventListener("add-cue", () => openCueEditor(null));
    toolbarSync.addEventListener("annotate", startAnnotationMode);
    toolbarSync.addEventListener("next-cue", jumpToNextCue);
    toolbarSync.addEventListener("rejoin", rejoinMaster);
...
if (settingsDialog) {
    settingsDialog.addEventListener("close", closeSettingsPanel);
    settingsDialog.addEventListener("rail-side-change", (e) => {
        railSide = e.detail.railSide;
        try {
            localStorage.setItem(RAIL_SIDE_STORAGE_KEY, railSide);
        } catch (_) {
        }
        applyDisplaySettings();
    });
    settingsDialog.addEventListener("margin-change", (e) => {
        queueDepartmentMarginSave(e.detail.side, e.detail.width);
    });
}
if (annotationToolbar) {
...
document.addEventListener('pointerdown', (e) => {
    if (settingsDialog && settingsDialog.open && e.target && e.target.closest &&
        e.target.closest('settings-dialog, #settingsPanel, #settingsBackdrop')) return;

    const inAnnotationUi = e.target && e.target.closest && e.target.closest('#annotationTools, annotation-toolbar');
    ...
    if (e.target && e.target.closest && e.target.closest('#annotationTools, annotation-toolbar, #toolbar')) return;
...
const {openExportPanel, closeExportPanel, startPdfExport} = createPdfExporter({
    exportDialog,
    ALLOWED_DEPARTMENTS, CUE_API_ENDPOINT,
...
if (toolbarDisplay) {
    toolbarDisplay.addEventListener("toggle-fullscreen", toggleFullscreen);
    ...
    toolbarDisplay.addEventListener("toggle-stage-directions", () => {
        ...
        if (toolbarDisplay) toolbarDisplay.showStageDirections = showStageDirections;
        applyStageDirectionVisibility();
    });
    toolbarDisplay.addEventListener("open-settings", openSettingsPanel);
...
viewport.addEventListener("scroll", () => {
    scheduleContextUpdate();
    if (!cueTrackingScrollPending && cueEditorDialog && cueEditorDialog.open && cuePositionTracking) {
