...
The export dialog markup remains in `teleprompter.html`, but its print-view
implementation is the `createPdfExporter` factory in `js/pdf-export.js`.
`js/main.js` supplies the dialog elements, endpoint constants, shared
normalization helpers, and current-state accessors, then wires the factory's
open, close, and start functions to the UI. Shared colour contrast logic comes
from `js/utils.js`, and semantic anchor validation is shared with
...
`css/teleprompter.css`. Changes to live screen styling therefore need an
